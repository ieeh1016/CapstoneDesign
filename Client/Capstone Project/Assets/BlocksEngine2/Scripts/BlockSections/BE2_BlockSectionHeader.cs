...
public class BE2_BlockSectionHeader : MonoBehaviour, I_BE2_BlockSectionHeader
{
    ...
    void Start()
    {
        ...
        BE2_MainEventsManager.Instance.StartListening(BE2EventTypes.OnPointerUpEnd, UpdateItemsArray);
        BE2_MainEventsManager.Instance.StartListening(BE2EventTypes.OnPointerUpEnd, UpdateInputsArray);
    }

    void OnDisable()
    {
        BE2_MainEventsManager.Instance.StopListening(BE2EventTypes.OnDrag, UpdateItemsArray);
        BE2_MainEventsManager.Instance.StopListening(BE2EventTypes.OnPointerUpEnd, UpdateItemsArray);
        BE2_MainEventsManager.Instance.StopListening(BE2EventTypes.OnPointerUpEnd, UpdateInputsArray);
    }
    ...
    {
        _inputsArray = new I_BE2_BlockSectionHeaderInput[0]; //new List<I_BE2_BlockSectionHeaderInput>();
        int childCount = transform.childCount;
        for (int i = 0; i < childCount; i++)
        {
            ...
            {
                BE2_ArrayUtils.Add(ref _inputsArray, input); //.Add(input);
            }
        }
    }
